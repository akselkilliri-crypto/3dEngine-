...
// ========== extrudeSelected (стабильная версия с поддержкой отрицательных значений) ==========
void HalfEdgeMesh::extrudeSelected(float distance) {
    ...
    for (int faceIdx : selectedFaceIndices) {
        ...
        if (outward) {
            ...
            // Вдавливание: исходная грань удаляется, создаётся внутренняя грань с правильной ориентацией
            Face innerFace;
...
// ========== ЗАГЛУШКИ ДЛЯ НОВЫХ МЕТОДОВ (чтобы не вылетало) ==========
void HalfEdgeMesh::deleteSelected() {
    LOGI("Delete selected (not implemented)");
}

void HalfEdgeMesh::smoothMesh() {
    LOGI("Smooth mesh (not implemented)");
}
